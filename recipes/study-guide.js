...
export const STUDY_GUIDE_RECIPE = {
  ...
  validate(content, fields) {
    ...
    if (
      ...
      return {
        ok: false,
        error: 'Missing or empty keyConcepts array.',
      };
    }
    ...
    for (const concept of content.keyConcepts) {
      if (
        ...
        return {
          ok: false,
          error:
            'A key concept is missing its term or explanation.',
        };
      }
    }

    if (
      !Array.isArray(content.summaryPoints) ||
      content.summaryPoints.length === 0
    ) {
      return {
        ok: false,
        error: 'Missing or empty summaryPoints array.',
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error:
          'Missing or empty practiceQuestions array.',
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: 'Missing or empty commonMistakes array.',
      };
    }
  ...
  toPlainTextParagraphs(content) {
    ...
    content.keyConcepts.forEach((c) => {
      lines.push(c.term + ': ' + c.explanation);
    });

    lines.push('');
    lines.push('Summary Points');

    content.summaryPoints.forEach((p) => {
      lines.push('- ' + p);
    });
